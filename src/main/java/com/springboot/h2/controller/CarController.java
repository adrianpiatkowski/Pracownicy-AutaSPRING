...
public class CarController {
    ...
    public String save(Model model, @ModelAttribute(value = "car") @Valid Car car, BindingResult result) {
        if (result.hasErrors() ) {
            model.addAttribute("empList", employeeService.getAll());
            return "car/car_form";
        }
    ...
    public ModelAndView edit(Model model , @RequestParam(value = "car_id") int car_id) {
        model.addAttribute("empList", employeeService.getAll());
        Car car = carService.getById(car_id);
